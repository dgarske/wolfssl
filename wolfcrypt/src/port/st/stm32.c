...
#ifndef NO_AES
#ifdef HAVE_AESGCM
/* Constant-time tag compare (no early-exit on a MAC value). 0 == equal. */
static int Stm32Dhuk_CtCmp(const byte* a, const byte* b, word32 n)
{
    byte   diff = 0;
    word32 i;
    for (i = 0; i < n; i++) {
...
static int Stm32Dhuk_Cipher(struct wc_CryptoInfo* info)
{
    ...
    case WC_CIPHER_AES_GCM:
        ...
        if (info->cipher.enc) {
            ...
            ret = Stm32Dhuk_CtCmp(tag, info->cipher.aesgcm_dec.authTag, tagSz);
            ForceZero(tag, sizeof(tag));
...
#ifdef WOLFSSL_STM32_CCB
/* ---------------------------------------------------------------------------
 * CCB (Coupling and Chaining Bridge) -- STM32U3 (e.g. U385), RM0487 ch 31.
 * The CCB chains PKA <-> SAES <-> RNG over a local interconnect so a
 * DHUK-protected private key is used by the PKA without ever entering software
 * or crossing the system bus. M0: bring the CCB up and confirm it is usable
 * (de-risks the GTZC / non-secure-state question before the OPSTEP work).
 * ------------------------------------------------------------------------- */
...
static int Stm32Ccb_SetParam(word32 slot, const byte* src, word32 sizeBytes)
{
    ...
    for (off = 0u; off < (operand - 2u); off += 2u) {
        ...
        PKA->RAM[slot + off]      = (word32)p[0] | ((word32)p[-1] << 8) |
                                    ((word32)p[-2] << 16) | ((word32)p[-3] << 24);
        PKA->RAM[slot + off + 1u] = (word32)p[-4] | ((word32)p[-5] << 8) |
    ...
    PKA->RAM[slot + ((sizeBytes + 3u) / 4u)]      = 0u;
    PKA->RAM[slot + ((sizeBytes + 3u) / 4u) + 1u] = 0u;
    return Stm32Ccb_SaesWaitCcf();
}
...
{
    PKA->RAM[slot]      = (word32)p[0] | ((word32)p[-1] << 8) |
                          ((word32)p[-2] << 16) | ((word32)p[-3] << 24);
    PKA->RAM[slot + 1u] = (word32)p[-4] | ((word32)p[-5] << 8) |
                          ((word32)p[-6] << 16) | ((word32)p[-7] << 24);
}
    ...
    const byte* wrapped, byte* pubX, byte* pubY)
{
    ...
    /* NOTE (C5 port): AES_CR_GCMPH (the SAES GCM-phase field, used here and in
     * the other CCB ops) does not exist on STM32C5 -- its SAES drives the GCM
     * phases by a different mechanism. This is the one genuinely-divergent part
     * of the OPSTEP driver and the main work item for a future C5 CCB arm; the
     * register *names* elsewhere are already family-abstracted (WC_STM32_CCB_*). */
    SAES->CR = (SAES->CR & ~AES_CR_GCMPH) | AES_CR_GCMPH_0 | AES_CR_EN;
    if ((ret = Stm32Ccb_WaitOpStep(0x13u)) != 0) { goto done; }

    if ((ret = Stm32Ccb_LoadCurve()) != 0) { goto done; }
    SAES->CR = (SAES->CR & ~AES_CR_GCMPH) | AES_CR_GCMPH_1;
    ...
    for (off = 0u; off < cipsz; off++) {
        ...
        if ((off % 4u) == 3u) {
            if ((ret = Stm32Ccb_SaesWaitCcf()) != 0) { goto done; }
            for (i = 0u; i < 4u; i++) {
                PKA->RAM[PKA_ECC_SCALAR_MUL_IN_K + block + i] = WC_CCB_MAGIC;
            }
            block += 4u;
        }
    }
    PKA->RAM[PKA_ECC_SCALAR_MUL_IN_K + cipsz]      = 0u;
    PKA->RAM[PKA_ECC_SCALAR_MUL_IN_K + cipsz + 1u] = 0u;
    if ((ret = Stm32Ccb_WaitOpStep(0x17u)) != 0) { goto done; }

    SAES->CR = (SAES->CR & ~AES_CR_GCMPH) | AES_CR_GCMPH_0 | AES_CR_GCMPH_1;
    SAES->DINR = 0u;
    ...
    wc_stm32_pka_read_be(pubX, &PKA->RAM[PKA_ECC_SCALAR_MUL_OUT_RESULT_X], 32u);
    wc_stm32_pka_read_be(pubY, &PKA->RAM[PKA_ECC_SCALAR_MUL_OUT_RESULT_Y], 32u);
    ret = 0;
    ...
    byte* pubX, byte* pubY)
{
    ...
    PKA->RAM[PKA_ECDSA_SIGN_IN_PRIVATE_KEY_D + off]      = 0u;
    PKA->RAM[PKA_ECDSA_SIGN_IN_PRIVATE_KEY_D + off + 1u] = 0u;
    ...
    block = 0u;
    for (off = 0u; off < cipsz; off++) {
        PKA->RAM[PKA_ECDSA_SIGN_IN_PRIVATE_KEY_D + off] = WC_CCB_MAGIC;
        if ((off % 4u) == 3u) {
            if ((ret = Stm32Ccb_SaesWaitCcf()) != 0) { goto done; }
            for (i = 0u; i < 4u; i++) {
    ...
    { word32 t = 0;
      while ((SAES->SR & AES_SR_BUSY) != 0u) {
          if (++t >= WC_STM32_CCB_TIMEOUT) { ret = WC_TIMEOUT_E; goto done; } } }
    PKA->RAM[PKA_ECDSA_SIGN_IN_PRIVATE_KEY_D + cipsz]      = 0u;
    PKA->RAM[PKA_ECDSA_SIGN_IN_PRIVATE_KEY_D + cipsz + 1u] = 0u;
    if ((ret = Stm32Ccb_PkaWaitFlag(PKA_SR_DATAOKF)) != 0) { goto done; }
    ...
    byte* r, byte* s)
{
    ...
    for (i = 0u; i < 8u; i++) {
        PKA->RAM[PKA_ECDSA_SIGN_IN_HASH_E + i] =
    ...
    for (off = 0u; off < cipsz; off++) {
        ...
        if ((off % 4u) == 3u) {
            if ((ret = Stm32Ccb_SaesWaitCcf()) != 0) { goto done; }
            for (i = 0u; i < 4u; i++) {
                PKA->RAM[PKA_ECDSA_SIGN_IN_PRIVATE_KEY_D + block + i] =
                    WC_CCB_MAGIC;
            }
            block += 4u;
        }
    ...
    for (off = 0u; off < (opsz - 2u); off++) {
        ...
        PKA->RAM[PKA_ECDSA_SIGN_IN_K + off] = WC_CCB_FAKE;
    }
    PKA->RAM[PKA_ECDSA_SIGN_IN_K + (opsz - 2u)]      = 0u;
    PKA->RAM[PKA_ECDSA_SIGN_IN_K + (opsz - 2u) + 1u] = 0u;
    ...
    if (PKA->RAM[PKA_ECDSA_SIGN_OUT_ERROR] != WC_CCB_PKA_OK) {
        ret = WC_HW_E;
        goto done;
    }
    wc_stm32_pka_read_be(r, &PKA->RAM[PKA_ECDSA_SIGN_OUT_SIGNATURE_R], 32u);
    wc_stm32_pka_read_be(s, &PKA->RAM[PKA_ECDSA_SIGN_OUT_SIGNATURE_S], 32u);
    ret = 0;
